...
app.post('/login', (req, res) => {
  ...
  if ((username === ADMIN_USERNAME && password === ADMIN_PASSWORD) || customAdmins.some(a => a.username === username && a.password === password)) {
...
const commands = [
  new SlashCommandBuilder().setName('ask').setDescription('Ask the AI a question').addStringOption(option => option.setName('question').setDescription('Your question').setRequired(true)),
  new SlashCommandBuilder().setName('create_acc').setDescription('Create admin credentials (owner only)')
].map(cmd => cmd.toJSON());
  ...
  try {
    if (interaction.commandName === 'ask') {
      ...
      if (data.choices && data.choices.length > 0) {
        ...
        await interaction.editReply('⚠️ Sorry, I couldn’t get a response from the AI.');
      }
    }
