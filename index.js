...
// Express Routes app.get('/', (req, res) => res.sendFile(path.join(__dirname, 'index.html')));
...
app.post('/login', (req, res) => { const { username, password } = req.body; const valid = username === 'Admin' && password === 'AI_KITTY' || customAdmins.find(a => a.username === username && a.password === password);

if (valid) { req.session.admin = username; res.json({ success: true }); } else { unauthorizedAttempts.push({ time: new Date(), username, password }); res.status(401).json({ success: false }); } });
...
Username: `${user}` Password: `${pass}``, ephemeral: true }); } } catch (err) { console.error('Interaction error:', err); try { await interaction.editReply('❌ Something went wrong.'); } catch {} } });
